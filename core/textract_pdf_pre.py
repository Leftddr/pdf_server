...
def textract_for_title(text):
    ...
    for index, text_char in enumerate(text):
        if index - 1 >= 0 and text[index - 1] == '\n' and (
                text_char >= '1' and text_char <= '9') and for_title == False:  # 소제목은 거의 1 introduction 이렇게 되어있다.
            for_title = True
        if for_title:
            tmp_str += text_char
            '''
...
def textract_for_big_title(text):
    ...
    for index, text_char in enumerate(text):
        if text_char == '%' and (((text[index + 1] >= 'A' and text[index + 1] <= 'Z') and (
                text[index - 1] >= 'a' and text[index - 1] <= 'z'))):  # Libnvmio : ~~~~~~~ \n Lib
            if len(pdf_title) == 0:
    ...
    if check_type(sys.argv[1]):
        ...
        for i in range(len(pdf_small_title)):
            if pdf_small_title[i] != 'References':
                textract_split_not_reference(pdf_small_title[i], text,
                                             text.find(pdf_small_title[i]) + len(pdf_small_title[i]), i)
            else:
                textract_split_reference(pdf_small_title[i], text,
                                         text.find(pdf_small_title[i]) + len(pdf_small_title[i]), i)
        ...
        #            src = kakao_check_language(content)
        #            translated_text = kakao_translate_language(content, src)
